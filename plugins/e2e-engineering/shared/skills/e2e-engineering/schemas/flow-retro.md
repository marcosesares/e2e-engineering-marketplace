...
## Sections
1. **§Local retro** — metrics for the team running the flow. Counters flight accumulates during the spawn: bounce count (by tier), blocked slices + cause (gate-3 exhaustion), [[Gate 5 failure]] count, stalls + reason, fan-out wave count (impl + review + verify), bounce rounds per slice against the cap of 4, verifier spend split by resolution, findings left `open-at-cap` and the followups they produced, and un-cited Minors dropped without verifier spend (ADR 0035). Operational, stays in the client project.
