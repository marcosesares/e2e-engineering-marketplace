...
3. **Gate 5 Failures → repair Tasks** — if `## Gate 5 Failures` section is present and non-empty, route each entry through [triage](../impl/triage.md) into a new repair Task (`parentTask=<this id>`, `status:needs-spec`, unselected). Human decides: real regression → bugfix Task; flaky/pre-existing → can drop. The built Task STILL goes `done` — Gate 5 failures are new scope, not a reason to reopen (ADR 0025).
4. **Followups → repair Tasks** — if `qa-signoff.md` `## Followups` is present and non-empty, route each entry through [triage](../impl/triage.md) (intake source #4) into a new repair Task (`parentTask=<this id>`, `status:needs-spec`, unselected), using each entry's own `suggestedPriority`. Entries under `## Release Blockers` are open `Critical` findings that merged at the bounce cap — route them P1 BEFORE approving sign-off. Source is `followups.json` (ADR 0035); the built Task still goes `done`, it is not reopened.
5. **Findings → issues** — log any QA finding in the Task's qa-signoff.md `## Findings`, then route each through [triage](../impl/triage.md) into a NEW queue Task: a **bug** → linked bugfix Task (`parentTask=<this id>`, `status:needs-spec`, unselected) — the built Task STILL goes `done`, not reopened (it passed automated gates; the bug is new scope); a **new idea** → feature Task (`status:needs-spec`, unselected). Findings re-enter the queue for a future flight (ADR 0018).
...
7. Record QA sign-off (set qa-signoff.md `Status: APPROVED`, queue `status: done`).
...
- `Followups` (only if `## Followups` non-empty): list each entry with its severity and suggested priority; call out `## Release Blockers` entries separately as P1-before-approval.
...
- Final ask: `Report pass/fail per TC, Gate 5 failure routing, followup + release-blocker routing, any findings, promote/drop per amendment, and which skill-improvement candidates to forward upstream.`
...
- Approving sign-off while `qa-signoff.md` `## Followups` or `## Release Blockers` still has un-routed entries (ADR 0035 — `followups.json` is the only record; an unrouted entry is lost when the Task goes `done`).
