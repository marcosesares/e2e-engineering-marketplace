...
## What to do
**Every command in this gate runs under [command-execution](./command-execution.md)** — bounded, non-interactive, self-terminating (ADR 0033). Stack-up 10 min, package build 15 min, suite 20 min. `compose up` ALWAYS `-d`; readiness by bounded poll (health endpoint / `compose ps`), NEVER by attaching to logs. A stack-up or suite timeout is a **gate-5 failure** (a strike), not a stall and not `blocked`.
