...
4. **Flight followups** from `tasks/<id>/followups.json` ([schema](../schemas/followups.json.md)) — expert-review findings still open when a slice exhausted its 4 bounce rounds (ADR 0035). Each becomes a NEW [Task queue](../schemas/queue.json.md) entry (`status:needs-spec`, unselected) with `parentTask=<built task id>` and priority from each entry's own `suggestedPriority` (`Critical` → 1, `Important`/`Minor` → 3; a file may hold a mix). Surfaced to the human via `qa-signoff.md` `## Followups` / `## Release Blockers`. Flight never creates the entry itself — ADR 0017 writer table.
