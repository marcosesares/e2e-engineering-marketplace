...
4. **Two-tier write probe (DSH-only).** Tier 1 — a REAL mutation, NEVER a config read: create + delete one throwaway file under the workspace root. A config read proves nothing, because the plan/phase contract is enforced ABOVE Permissions and Sandbox (see Host gate layers). `reasonix.toml` `[permissions] mode` is a fast NEGATIVE only — `ask`/`deny` → stall early; `allow` advances nothing on its own. Tier 2 — spawn ONE disposable write worker with the SAME `write_paths` as real impl workers, brief: create + commit a probe file on a throwaway branch. Commit visible from orchestrator → worker writes confirmed. Worker lacks `bash`/writer (permissions config cannot override runtime worker-stripping) → record `resume.json mode: serial` and fly serial (A) — NEVER preset serial. On denial, CLASSIFY by the exact host message BEFORE stalling (ADR 0040): `constraints forbid state mutation` / `forbid verification commands` / `forbid push/publish/deploy-style actions` / `plan mode forbids workspace mutations` → `<e2e-stall reason="plan-contract-active" />` + EXIT — layer 1, the operator clears it in-session (exit Plan mode / approve the plan); NEVER reword it as a sandbox or permissions fault. Any OTHER repeated write denial → `<e2e-stall reason="sandbox-write-denied" />` + EXIT. Both are FINAL — never retry/loop; a single denial before classification is diagnosis, not a stall.
...
Three independent gates, outermost first (ADR 0040). A block from an outer layer NEVER reaches the inner ones — so an inner-layer config can read as fully permissive while every write fails. Always classify by the host's exact message, never by what a config file says.

1. **Plan/phase contract (outermost).** Messages: `blocked: plan mode forbids workspace mutations until the plan is approved`, `blocked: the current constraints forbid state mutation`, `... forbid verification commands`, `... forbid push/publish/deploy-style actions`. Enforced BEFORE Permissions and Sandbox (the host applies explicit phase opt-outs first). No config value reaches it and no approval posture lifts it — YOLO/`bypassPermissions` toggles layer 2, which the request never gets to. Only the operator clears it in-session, by exiting Plan mode or approving the plan. Stall `plan-contract-active`, never `sandbox-write-denied`.
2. **Permissions.** Two sub-layers that must BOTH allow. (a) `reasonix.toml [permissions]` — `mode` is the rule-miss fallback, plus the `allow`/`ask`/`deny` rules. (b) The session runtime permission mode (`--permission-mode`, default `ask`), which the config file CANNOT set. A worker has no interactive approver, so `ask` fails closed for every worker even when the orchestrator is interactive.
3. **Sandbox (innermost).** File writers confined to `workspace_root` + `allow_write`. read-only → PowerShell ConstrainedLanguage: .NET statics, `Add-Type`, COM, named-pipe stdout capture FAIL; cmdlets + core types work. workspace-write → FullLanguage; flight work (git commit, build outputs) needs this at minimum. On Windows the OS-level bash jail is fixed `off`, so `[sandbox] bash` is not the denial source there.
