...
Durable project docs at repo ROOT: `CONTEXT.md` (glossary), [constitution](../../shared/skills/e2e-engineering/constitution.md) (generic engineering standards), `ARCHITECTURE.md` (project-specific structure + conventions — schema: [architecture](../../shared/skills/e2e-engineering/schemas/architecture.md)), `DESIGN.md` (project design system — register, tokens, components — schema: [design](../../shared/skills/e2e-engineering/schemas/design.md)). ARCHITECTURE.md + DESIGN.md written ONLY in human phases (pre-impl seed + post-impl human-QA amend); implementation loop reads them, never writes. See ADR 0013, ADR 0030.
...
Sequence (bracketed = conditional): **[map-codebase? (brownfield)] → grill-with-docs → [research?] → [prototype?] → [design? (has-ui)] → to-prd**. (ADR 0019, ADR 0030.)
...
5. [design](../../shared/skills/e2e-engineering/pre-impl/design.md) — only if the task bears UI (the `design?` gate). Sets register (Brand vs Product), seeds/records `DESIGN.md` (schema: [design](../../shared/skills/e2e-engineering/schemas/design.md)) against the [ui-design standard](../../shared/skills/e2e-engineering/standards/ui-design.md). Human-phase-written; flight reads it. Greenfield drafts (`<!-- SEED -->`); brownfield records reality + flags deviations.
6. [to-prd](../../shared/skills/e2e-engineering/pre-impl/to-prd.md) — convert grill-with-docs notes into formal PRD → writes `prd.json`. Owns own interview step (no double-interview). Refactor-shaped stories allowed. Captures testing-decisions → test-cases.
...
2. **Fan-out** — dispatch each ready story to OWN git worktree + subagent (`EnterWorktree`). Inject [constitution](../../shared/skills/e2e-engineering/constitution.md) + [testing standard](../../shared/skills/e2e-engineering/standards/testing.md) + story (incl. `integration` decision) + testCases. Per sliceType: schema/db → + [db standard](../../shared/skills/e2e-engineering/standards/db.md); api/logic → + [api-testing standard](../../shared/skills/e2e-engineering/standards/api-testing.md) + [backend standard](../../shared/skills/e2e-engineering/standards/backend.md); ui → + [ui-design standard](../../shared/skills/e2e-engineering/standards/ui-design.md) + SCOPED slice of DESIGN.md. Brownfield / ARCHITECTURE.md exists → also inject story's SCOPED slice of ARCHITECTURE.md (§Index for offset/limit — this layer's naming + ownership + anti-patterns, NOT whole doc). Subagent runs [tdd](../../shared/skills/e2e-engineering/impl/tdd.md): gap-check → red-green-refactor (unit + API/integration via Playwright `request`; UI Manual, no automation — Fork Y) → return compact manifest with `evidencePaths[]`.
...
3. **Fan-in (orchestrator, serial — sole writer):** per returned summary, run per-slice review's two ordered stages, then merge-readiness check, then merge.
   ...
   - **Bounce ceiling (token-runaway guard).** Stage-1 or stage-2 bounce capped at **3 round-trips**. Still failing after 3 → STOP: mark story `blocked`, append `## Blocked` in progress.txt, tear down worktree, keep draining. _Legacy single-Task inline path only; superseded for `/e2e-flight` by ADR 0035's review convergence loop (cap 4, review findings never produce `blocked`, residue rides `followups.json`)._
